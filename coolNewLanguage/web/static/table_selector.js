async function table_sel_show_table(
	table_name,
	component_id,
	context,
	table_transient_id
...
function table_sel_hide_full_table(component_id, table_transient_id) {
	const full_table_div = document.getElementById(
		`table_select_full_table_${component_id}_table_${table_transient_id}`
	);
	full_table_div?.remove();
	// unstyle the preview of the table as selected
	const button = table_sel_get_table_select_button(
		component_id,
		table_transient_id
	);
...
function table_sel_confirm_table_choice(
	table_name,
	component_id,
	table_transient_id
) {
	...
	const table_preview_checkboxes = document.querySelectorAll(
		`input.table_preview_checkbox[data-component-id="${component_id}"]`
	);
	// check/uncheck them appropriately
	for (const tablePreviewCheckbox of table_preview_checkboxes) {
		tablePreviewCheckbox.checked =
			tablePreviewCheckbox.id ===
			`table_preview_checkbox_${table_transient_id}_${component_id}`;
	}
	// hide the table
