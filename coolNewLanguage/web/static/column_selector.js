...
async function col_sel_show_table(
	table_name,
	component_id,
	context,
	table_transient_id
	...
	const response = await fetch(
		`/_get_table?table=${table_name}&context=${context}&component_id=${component_id}&table_transient_id=${table_transient_id}`
	);
	const table_html = await response.text();
	// if a table is already being shown, unstyle its preview and delete it from the dom
	const table_selector_div = document.getElementById(
		`table_select_${component_id}`
	);
	const next_element = table_selector_div.nextElementSibling;
	if (
		next_element !== null &&
		next_element.classList.contains("table_select_full_table")
	) {
		next_element.remove();
		// get the preview element and unstyle it
		const table_preview = table_selector_div.querySelector(
			"button.table_select_button_selected"
		);
	...
	console.log("Trying to convert full table to dataTable");
	$("div.table_select_full_table table").DataTable();
...
function col_sel_hide_full_table(component_id, table_transient_id) {
	const full_table_div = document.getElementById(
		`column_select_full_table_${component_id}_table_${table_transient_id}`
	);
	full_table_div.remove();
	// unstyle the preview of the table as selected
	const button = col_sel_get_table_select_button(
		component_id,
		table_transient_id
	);
...
function col_sel_toggle_column_selection(
	col_name,
	col_index,
	component_id,
	table_transient_id
	...
	const col_cells = document.querySelectorAll(
		`#column_select_full_table_${component_id}_table_${table_transient_id} .col_${col_index}`
	);
...
function col_sel_toggle_column_as_hovered(
	col_index,
	component_id,
	table_transient_id
) {
...
function col_sel_toggle_column_as_unhovered(
	col_index,
	component_id,
	table_transient_id
) {
...
function col_sel_confirm_column_choices(component_id, transient_table_id) {
	...
	const table_name_input = document.getElementById(
		`input_${component_id}_table_name`
	);
	table_name_input.value = transient_choices.get("table");
	// Remove any existing column choice inputs
	const existing_col_choice_inputs = document.querySelectorAll(
	...
	const selected_table_p = document.getElementById(
		`col_sel_selected_table_${component_id}`
	);
	selected_table_p.hidden = false;
	selected_table_p.innerText = `Selected table: ${table_name_input.value}`;
	const selected_columns_p = document.getElementById(
		`col_sel_selected_columns_${component_id}`
	);
